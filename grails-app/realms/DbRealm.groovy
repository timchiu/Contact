...
//        def user = User.findByUsername(username)
		def user = TaoMember.findByUsername(username)
        if (!user) {
		...
		def account = new SimpleAccount(user, user.passwordHash, "DbRealm")
...
//        def user = User.findByUsername(principal)
        ...
        def results = User.executeQuery("select distinct p from User as user join user.roles as role join role.permissions as p where user.username = '${principal.username}'")
